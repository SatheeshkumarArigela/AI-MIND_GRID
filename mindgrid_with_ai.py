...
def is_valid_location(board, col):
	return board[GRID_ROW_COUNT-1][col] == 0
...
def score_position(board, piece):
	...
	## Score center column
	center_array = [int(i) for i in list(board[:, GRID_COLUMN_COUNT//2])]
...
def get_valid_locations(board):
	valid_locations = []
	for col in range(GRID_COLUMN_COUNT):
...
width = GRID_COLUMN_COUNT * SQUARESIZE
height = (GRID_ROW_COUNT+1) * SQUARESIZE
...
while not game_over:
	...
	if turn == AI and not game_over:				
		...
		#col = random.randint(0, GRID_COLUMN_COUNT-1)
